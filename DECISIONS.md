...
**Milestone:** We intend to test the engine across a much larger window (100K+ txns) once deployed in a shadow mode on real IEEE-CIS data. The current 12K txn holdout with 0.4% fraud yields only ~12 frauds in the weakest slice, which is too small for statistical significance. We need a 100K+ window to properly validate the O(1) state management and model performance under load.
